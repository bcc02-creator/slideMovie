...
class SlidecastAudio {
  ...
  setBgmLevels({ base, duck }) {
    ...
    if (this.bgmGain && this.playing && this.bgmSource) {
      const g = this.bgmGain.gain;
      const now = this.ctx.currentTime;
      g.cancelScheduledValues(now);
      g.linearRampToValueAtTime(this.bgmBaseLevel, now + 0.2);
    }
  }

  // Standalone BGM playback (used in TTS mode where play() isn't called)
  playBgmOnly() {
    this.ensureCtx();
  ...
  stopBgmOnly() {
    if (this.bgmSource) {
      ...
      try { this.bgmSource.disconnect(); } catch {}
      this.bgmSource = null;
    }
  ...
  play(fromOffset = 0) {
    ...
    if (this.bgmBuffer) {
      ...
      g.setValueAtTime(0, now);
      g.linearRampToValueAtTime(this.bgmBaseLevel, t0 + 0.6);
      // fade out at end of VO — only if there's actual VO content past fromOffset
      const remainingVo = this.totalVoDuration - fromOffset;
      if (remainingVo > 1.5) {
        const endAt = t0 + remainingVo;
        g.setValueAtTime(this.bgmBaseLevel, endAt - 1.0);
        g.linearRampToValueAtTime(0, endAt + 0.2);
      }
      // else: no VO (or already past it) — let BGM loop indefinitely at base level
    }
...
Object.assign(window, { SlidecastAudio, SlidecastRecorder });
